    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (version.startsWith("v1.")) {
            ...
            if (punto>0){
                ...
                if (subVersion <= 37){ //Si la versión que tiene instalada la tablet es menor o igual a la v1.37 se intentan añadir las columnas.
                    addColumn(db, Contracts.TABLE_CUEPASAJEROS, Contracts.COLUMN_CUEPASAJEROS_PLAYA);
                    addColumn(db, Contracts.TABLE_CUEPASAJEROS, Contracts.COLUMN_CUEPASAJEROS_SITIOPARKOTRO);
                }
                if (subVersion <= 41){ //Si la versión que tiene instalada la tablet es menor o igual a la v1.41 se intentan añadir las columnas.
                    addColumn(db, Contracts.TABLE_CUEPASAJEROS, Contracts.COLUMN_CUEPASAJEROS_VTC);
                    addColumn(db, Contracts.TABLE_CUEPASAJEROS, Contracts.COLUMN_CUEPASAJEROS_VTCOTRO);
                }
            }
        }
